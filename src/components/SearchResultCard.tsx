...
const SearchResultCard = ({ restaurant }: Props) => {
  return (
    <Link
      ...
      className='grid lg:grid-cols-[2fr_3fr] gap-5 group border border-gray-300 rounded-md overflow-hidden bg-[#e3f2fd]'
    >
        ...
        <div id='card-content' className='grid md:grid-cols-2 gap-2'>
          <div className='flex flex-row flex-wrap'>
            {restaurant.cuisines.map((item, index) => (
              <span className='flex'>
                <span className='font-bold text-cyan-500'>{item}</span>
